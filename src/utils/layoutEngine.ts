...
function buildElkGraph(
  ...
  for (const edge of edges) {
    ...
    if (
      sourceTop !== targetTop &&
      topLevelIds.has(sourceTop) &&
      topLevelIds.has(targetTop)
    ) {
...
function estimatePathCost(
  ...
  let cost =
    Math.abs(targetPos.x - sourcePos.x) + Math.abs(targetPos.y - sourcePos.y);
  ...
  for (const obs of obstacles) {
    ...
    if (overlapsX && overlapsY) {
      ...
      const detourX = Math.max(
        0,
        Math.min(maxX, obsRight) - Math.max(minX, obsLeft),
      );
      const detourY = Math.max(
        0,
        Math.min(maxY, obsBottom) - Math.max(minY, obsTop),
      );
...
function optimizeAutoEdges(
  ...
  for (const edge of edges) {
    ...
    const edgeObstacles = allObstacles.filter(
      (obs) => obs !== sourceRect && obs !== targetRect,
    );

    const { sourceSide, targetSide } = getBestSide(
      sourceRect,
      targetRect,
      edgeObstacles,
    );
  ...
  const getMiddleSegment = (info: EdgeInfo): { x: number; y: number } => {
    ...
    if (
      (info.sourceSide === "right" && info.targetSide === "left") ||
    ...
    if (
      (info.sourceSide === "top" && info.targetSide === "bottom") ||
  ...
  const getPathSignature = (info: EdgeInfo): string => {
    ...
    if (
      ...
      const maxY = Math.max(
        info.sourceRect.y + info.sourceRect.height,
        info.targetRect.y + info.targetRect.height,
      );
    ...
    if (
      ...
      const maxX = Math.max(
        info.sourceRect.x + info.sourceRect.width,
        info.targetRect.x + info.targetRect.width,
      );
  ...
  const doSegmentsOverlap = (info1: EdgeInfo, info2: EdgeInfo): boolean => {
    ...
    if (
      (info1.sourceSide === "right" && info1.targetSide === "left") ||
      ...
      const y1Range = [
        Math.min(info1.sourceRect.y, info1.targetRect.y),
        Math.max(
          info1.sourceRect.y + info1.sourceRect.height,
          info1.targetRect.y + info1.targetRect.height,
        ),
      ];
      const y2Range = [
        Math.min(info2.sourceRect.y, info2.targetRect.y),
        Math.max(
          info2.sourceRect.y + info2.sourceRect.height,
          info2.targetRect.y + info2.targetRect.height,
        ),
      ];
    ...
    if (
      (info1.sourceSide === "bottom" && info1.targetSide === "top") ||
      (info1.sourceSide === "top" && info1.targetSide === "bottom")
    ) {
      const yClose = Math.abs(mid1.y - mid2.y) < OVERLAP_THRESHOLD;
      const x1Range = [
        Math.min(info1.sourceRect.x, info1.targetRect.x),
        Math.max(
          info1.sourceRect.x + info1.sourceRect.width,
          info1.targetRect.x + info1.targetRect.width,
        ),
      ];
      const x2Range = [
        Math.min(info2.sourceRect.x, info2.targetRect.x),
        Math.max(
          info2.sourceRect.x + info2.sourceRect.width,
          info2.targetRect.x + info2.targetRect.width,
        ),
      ];
    ...
    return (
      Math.abs(mid1.x - mid2.x) < OVERLAP_THRESHOLD &&
      Math.abs(mid1.y - mid2.y) < OVERLAP_THRESHOLD
    );
  };
  ...
  for (const group of mergedGroups) {
    if (group.length === 1) {
      ...
      const sorted = [...group].sort((a, b) => {
        ...
        if (
          (a.sourceSide === "right" && a.targetSide === "left") ||
        ...
        if (
          (a.sourceSide === "bottom" && a.targetSide === "top") ||
  ...
  for (const info of edgeInfos) {
    ...
    const sourceIndex =
      sourceHandleMap.get(edge.id) ?? Math.floor(HANDLES_PER_SIDE / 2);
    const targetIndex =
  ...
  const unifiedHandleGroups = new Map<
    string,
    { edge: ServiceEdge; isSource: boolean }[]
  >();
