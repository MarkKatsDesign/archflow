...
function findBestMidpoint(
  ...
  if (isHorizontalFirst) {
    ...
    const escapeX =
      (goingRight
        ? allObstacleBounds.right + 20
        : allObstacleBounds.left - 20) + corridorOffset;
    ...
    const farEscapeX =
      (goingRight
        ? allObstacleBounds.right + 50
        : allObstacleBounds.left - 50) + corridorOffset;
    ...
    const oppositeEscapeX =
      (goingRight
        ? allObstacleBounds.left - 50
        : allObstacleBounds.right + 50) + corridorOffset;
    ...
    if (immediateBlockers.length > 0) {
      ...
      for (const { escapeY } of escapeDirections) {
      ...
      const veryFarEscapeX =
        (goingRight
          ? allObstacleBounds.right + 100
          : allObstacleBounds.left - 100) + corridorOffset;
      for (const { escapeY } of escapeDirections) {
        const esc1: Point = { x: source.x, y: escapeY };
      ...
      for (const { escapeY } of escapeDirections) {
        ...
        const approachOffsets = [
          { approachY: target.y - 50 },
          { approachY: target.y + 50 },
          { approachY: target.y - 100 },
          { approachY: target.y + 100 },
        ];

        for (const { approachY } of approachOffsets) {
    ...
    const escapeY =
      (goingDown ? allObstacleBounds.bottom + 20 : allObstacleBounds.top - 20) +
      corridorOffset;
    ...
    // Try further out
    const farEscapeY =
      (goingDown ? allObstacleBounds.bottom + 50 : allObstacleBounds.top - 50) +
      corridorOffset;
    ...
    const oppositeEscapeY =
      (goingDown ? allObstacleBounds.top - 50 : allObstacleBounds.bottom + 50) +
      corridorOffset;
    ...
    const immediateBlockers = findBlockingObstacles(
      source,
      {
        x: source.x,
        y: goingDown
          ? allObstacleBounds.bottom + 50
          : allObstacleBounds.top - 50,
      },
      obstacles,
    );
    ...
    if (immediateBlockers.length > 0) {
      ...
      const escapeDirections = [
        { escapeX: blockerBounds.right + 15 },
        { escapeX: blockerBounds.left - 15 },
      ];

      for (const { escapeX } of escapeDirections) {
      ...
      for (const { escapeX } of escapeDirections) {
        const approachOffsets = [
          { approachX: target.x - 50 },
          { approachX: target.x + 50 },
          { approachX: target.x - 100 },
          { approachX: target.x + 100 },
        ];

        for (const { approachX } of approachOffsets) {
          const esc1: Point = { x: escapeX, y: source.y };
          const esc2: Point = { x: escapeX, y: farEscapeY };
  ...
  if (isHorizontalFirst) {
    ...
    for (const gap of gaps) {
      ...
      if (gapSize >= 40) {
        ...
        const escapeX =
          (goingRight
            ? allObstacleBounds.right + 30
            : allObstacleBounds.left - 30) + corridorOffset;
    ...
    for (const gap of gaps) {
      ...
      if (gapSize >= 40) {
        ...
        const escapeY =
          (goingDown
            ? allObstacleBounds.bottom + 30
            : allObstacleBounds.top - 30) + corridorOffset;
...
function calculatePCBPath(
  ...
  const sourceSide =
    ...
    inferHandleSide(true, sourceX, sourceY, targetX, targetY);
  const targetSide =
    targetHandle?.split("-")[0] ||
    inferHandleSide(false, sourceX, sourceY, targetX, targetY);
